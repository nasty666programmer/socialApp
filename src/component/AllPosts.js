...
function AllPosts({posts,props}) {
    const [flag,setFlag] = useState(false);
     function TrueLike(userName,firstname,text) {
        setFlag(!flag)
        let btn = document.querySelector('.likes_btn');
        if(flag){
            btn.classList.add('likes')
            alert('this posts add in your saved')
           
        }
        else {
            alert('this posts delete in your saved')
        btn.classList.remove('likes')
        }
    }
    return (
        <div>
